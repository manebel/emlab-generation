...
meritOrderPlot<-ggplot(bigDF, aes(x=capacity, y=costs))+
  ...
  scale_x_continuous(lim=c(0,400),
   ...
   #geom_text(data=bigDF[1510,],label="Renew.", position=position_dodge(width=1), vjust=0,hjust=1,size=2.5,angle = 90) +
#   annotation_custom(
#     grob = textGrob(label = "Renew.", rot=90,hjust = 0.1,vjust=0, gp = gpar(cex = 0.4)),
#     ymin = 5,      # Vertical position of the textGrob
#     ymax = 5,
#     xmin = 40,         # Note: The grobs are positioned outside the plot area
#     xmax = 40)+
#   annotation_custom(
#     grob = textGrob(label = "Nu.", rot=90,hjust = 0.1,vjust=0, gp = gpar(cex = 0.4)),
#     ymin = 5,      # Vertical position of the textGrob
#     ymax = 5,
#     xmin = 74,         # Note: The grobs are positioned outside the plot area
#     xmax = 74)+
#   annotation_custom(
#     grob = textGrob(label = "Coal", rot=90,hjust = 0.1,vjust=0, gp = gpar(cex = 0.4)),
#     ymin = 10,      # Vertical position of the textGrob
#     ymax = 10,
#     xmin = 120,         # Note: The grobs are positioned outside the plot area
#     xmax = 120)+
#   xlab("\nCapacity")+  
#   annotation_custom(
#     grob = textGrob(label = "CCGT", rot=0,hjust = 0.1,vjust=0, gp = gpar(cex = 0.4)),
#     ymin = 20,      # Vertical position of the textGrob
#     ymax = 20,
#     xmin = 215,         # Note: The grobs are positioned outside the plot area
#     xmax = 215)+
#   xlab("\nCapacity")+ # X- axis      
#   annotation_custom(
#     grob = textGrob(label = "OCGT", rot=0,hjust = 0.1,vjust=0, gp = gpar(cex = 0.4)),
#     ymin = 30,      # Vertical position of the textGrob
#     ymax = 30,
#     xmin = 310,         # Note: The grobs are positioned outside the plot area
#     xmax = 310)+
  xlab("\nTechnology")+ # X- axis  
...
ggsave(filename="meritOrder-DemandRisk3.pdf",plot=meritOrderPlot, width=20, height=20, units="cm")