...
GenericInvestmentRole<T> implements Role<T>, NodeBacked {
    ...
    public void act(T agent) {
        ...
        // Get a collection (dataCollection)
        // here, that has all datapoints(ticks), substance and price, which will
        ...
        // CHANGE: create scenarios for risk evaluation
        if (riskConsideration) {
            // TODO// List<Map<Substance, Double>> scenarios =
            // createScenarios(agent, futureTimePoint);// TODO
        }
        ...
        for (PowerGeneratingTechnology technology : reps.genericRepository.findAll(PowerGeneratingTechnology.class)) {
            ...
            if ((expectedInstalledCapacityOfTechnology + plant.getActualNominalCapacity())
                ...
                if (runningHours < plant.getTechnology().getMinimumRunningHours()) {
                    ...
                    // based on the companies debt-ratio -> This shold be done
                    // outside of the loop over
                    // all technologies
                    ...
                    // TODO: Alle Projekte mit projectValue > 0 merken und
                    // sortieren (sorted list of technologies). Dazu dann noch
                    // weitere Informationen speichern,
                    // so dass Neuberechnung nicht immer erforderlich ist
        ...
        if (bestTechnology != null) {
            ...
            // TODO: Hier den Risikocheck machen
            // in einer Schleife über alle Technologien gehen
            // (beginnend mit der besten) und solange Risiko-check
            // machen, bis dieser bestanden wurde.

            // NPV Berechnung evtl. auslagern in eigener Methode, die hier
            // aufegrufen wird?
            // Wiederbenutzung verschiedener Informationen?

            // logger.warn("Agent {} invested in technology {} at tick " +
            // getCurrentTick(), agent, bestTechnology);

            // Zwei Funktionen:
            // 1. createScenarios(agent, forecastedValues, marketInformation)
            // --- Beginn Schleife ---
            // 2. evaluateTechnologyUnderscenarios(scenarios, technology,
            // agent,...)
            // --- Ende Schleife ---
